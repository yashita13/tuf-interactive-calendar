...
export function DayCell({ date, status, hasNotes, hasEvents, onClick }: DayCellProps) {
  ...
  if (!isCurrentMonth) {
    return (
      <div className="h-10 w-10 md:h-12 md:w-12 flex items-center justify-center text-gray-text/10 text-sm md:text-base font-bold pointer-events-none transition-colors duration-500">
        {format(date, 'd')}
      </div>
    );
  }
  ...
  return (
    <motion.button
      ...
      className={cn(
        "relative h-10 w-10 md:h-12 md:w-12 flex items-center justify-center text-sm md:text-base font-bold transition-all duration-300 rounded-xl",
